...
def topology_idn(**kwargs): 
    ...
    #===================================================================================
    # receivers = [n for n in topology.nodes() if deg[n] == 2]
    # sources_real = [n for n in topology.nodes() if deg[n] == ]
 
#=====================================================
@register_topology_factory("PALAPARING")
def topology_idn(**kwargs): 
    topology = fnss.parse_topology_zoo(
        path.join(TOPOLOGY_RESOURCES_DIR, "palaparing.graphml")
    ).to_undirected()
    topology = largest_connected_component_subgraph(topology)
    deg = nx.degree(topology)
    #FIX SABI INI MAH 
    sources_asli = [6, 8, 25, 30, 34, 38, 75, 72, 66, 82, 83, 47]
    sources = []
    for v in sources_asli:
        u = v + 1000  # node ID of source
        topology.add_edge(v, u)
        sources.append(u)
    print("NODE PRODUCER")
    print(sources)
    receivers = [0, 12, 15, 46, 10, 32, 40, 78, 74, 67, 59, 55 ]
    print("CUSTOMER")
    print(receivers)
    routers = [n for n in topology.nodes() if n not in sources + receivers]
    #routers = []
    icr_candidates = routers
    print("NODE ROUTER CACHE")
    print(icr_candidates)
    # set weights and delays on all links
    fnss.set_weights_constant(topology, 1.0)
    fnss.set_delays_constant(topology, INTERNAL_LINK_DELAY, "ms")
