...
from icarus.models.strategy.criticcocoso import criticcocoso
from icarus.models.strategy.ewmtopsis import ewmtopsis_v1, ewmtopsis_v2
...
class mcdm(Strategy):
    ...
    def __init__(self, view, controller, use_ego_betw=False, **kwargs):
        ...
        for y in self.betw:
            if view.has_cache(y):
                self.tablenit[y]=[self.betw[y],0,0,0] #BC,CR,ACC,HOP
                self.tablecit[y]=[]
                self.tablec[y]=[]
        self.nodecachesize = view.cache_nodes(True)
    ...
    def process_event(self, time, receiver, content, log):
        ...
        for u, v in path_links(path):
            ...
            if self.view.has_cache(v):
                self.tablenit[v][3] = hop
                self.tablenit[v][2] = self.nodecachesize[v] - len(self.view.cache_dump(v))
                if self.controller.get_content(v):
                    if content not in self.tablec[v]:
                        self.tablec[v].append(content)
                        self.tablecit[v].append([content, 1])
                    else:
                        for i in range(len(self.tablecit[v])):
                            if self.tablecit[v][i][0] == content:
                                self.tablecit[v][i][1] += 1
                                ind = i
                                break
                        if ind > 0:
                            for j in reversed(range(ind)):
                                if self.tablecit[v][j+1][1] > self.tablecit[v][j][1]:
                                    temp1 = self.tablecit[v][j+1]
                                    self.tablecit[v][j+1] = self.tablecit[v][j]
                                    self.tablecit[v][j] = temp1
                                    temp2 = self.tablec[v][j+1]
                                    self.tablec[v][j+1] = self.tablec[v][j]
                                    self.tablec[v][j] = temp2
                                else:
                                    break
        ...
        vcand = [node for node in path if self.view.has_cache(node)]
        vcandict ={}
        for x in vcand:
            vcandict[x]=self.tablenit[x]
        vc = None
        if len(vcand) > 0:
            if serving_node == source:
                vc = vcand[0]
            elif serving_node != vcand[-1]:
                for x in range(int(len(self.tablec[serving_node])*0.2+1)):
                    if content == self.tablec[serving_node][x]:
                        vc = criticcocoso(vcandict)
                        break
        ...
        for u, v in path_links(path):
            ...
            if v == vc:
                # insert content
                cond = self.controller.put_content(v)
                if cond != None:
                    self.tablenit[v][1] += 1
